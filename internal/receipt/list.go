...
// ListItem is one entry in a List. Content is plain text, not nested
// Elements (the flat design Table's Headers/Rows also use). Checked is
// only meaningful when the parent List.Kind is "checkbox". Indent is a
// semantic nesting level (0 = top level), not a character count — how much
// visual space a level occupies is a rendering choice, not part of this
// type's contract. See docs/adr/0014-list-elements.md.
...
// maxListIndent bounds ListItem.Indent against input that would consume
// the entire printable width in leading space before any content renders.
// The exact bound is deliberately unspecified by
// docs/adr/0014-list-elements.md, so it is unexported and may be tuned
// without a schema-visible or architectural change.
const maxListIndent = 8

// Validate reports whether l is well-formed: Kind must be "", "bullet",
// "number", or "checkbox"; Items must be non-empty; and every item's
// Content must be non-empty and valid UTF-8, Indent in [0, maxListIndent],
// and Checked false unless Kind is "checkbox". A Checked item under a
// non-checkbox Kind is rejected outright rather than silently ignored.
...
// MarshalJSON encodes l with the "type":"list" discriminator the registry
